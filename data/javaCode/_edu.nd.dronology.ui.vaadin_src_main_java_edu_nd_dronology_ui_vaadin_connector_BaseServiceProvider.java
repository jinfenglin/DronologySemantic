...
public class BaseServiceProvider implements IBaseServiceProvider {
	...
	public IRemoteManager getRemoteManager() throws DronologyServiceException {
		try {
			...
			System.out.println(">> Fetching Remote Manager from:  '" + String.format(ADDRESS_SCHEME, ip, port));
			manager = (IRemoteManager) Naming.lookup(String.format(ADDRESS_SCHEME, ip, port));
			isConnected = true;
			return manager;
